    ...
    def nll_localisation(
        ...
        if self.double_pass and pass_count == 0:
            ...
            if cat2:
                ...
                # Pass 3: if MAD outliers remain in Pass 2 result, relocate without them.
                # NLL may assign low time_weight to picks that survived inter-pass cleanup;
                # a cosmetic zeroing would not fix the location, so we re-run NLL.
                mad_outliers_pass2 = self._detect_mad_outliers(event2, orig2)
                if mad_outliers_pass2:
                    outlier_pick_ids = {a.pick_id for a in mad_outliers_pass2}
                    picks3 = [p for p in event2.picks if p.resource_id not in outlier_pick_ids]
                    logger.info(
                        f"Pass 3 (MAD post-filter): {len(mad_outliers_pass2)} outlier(s) removed, "
                        f"{len(picks3)} picks remaining — relaunching NLL."
                    )
                    if len(picks3) >= self.nll_min_phase:
                        event3_tmp = Event(picks=picks3)
                        cat3_tmp = Catalog(events=[event3_tmp])
                        new_nll_obs_file_3 = nll_obs_file + ".3rd_pass"
                        cat3_tmp.write(new_nll_obs_file_3, format="NLLOC_OBS")
                        try:
                            cat3 = self.nll_localisation(
                                new_nll_obs_file_3,
                                picks=picks3,
                                double_pass=self.double_pass,
                                pass_count=1,
                                ...
                                force_template=nll_template,
                                force_loc_method=loc_method_used,
                                _pass_label="Pass 3 (MAD post-filter)",
                            )
                        except Exception as ex3:
                            logger.warning(f"Pass 3 relocation failed: {ex3} — keeping Pass 2 result.")
                            cat3 = None
                        if cat3:
                            event2 = cat3.events[0]
                            orig2 = event2.preferred_origin()
                            self._remap_arrivals_to_existing_picks(orig2, event2, e)
                    else:
                        logger.warning(
                            f"Pass 3 skipped: only {len(picks3)} picks remaining "
                            f"(< nll_min_phase={self.nll_min_phase}) — keeping Pass 2 result."
                        )
