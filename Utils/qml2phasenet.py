...
def export_picks_to_phasenet_format(event, origin):
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(columns=["seedid", "phasename", "time", "probability", "eventid"])
    for event in cat.events:
        origin = event.preferred_origin()
        picks_list = export_picks_to_phasenet_format(event, origin)
